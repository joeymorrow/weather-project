...
def run_sync():
    try:
        ...
        pulse_task = "Task 2 (Pulse): 1-sentence sleek summary of tomorrow's weather forecast. Start with 'Tomorrow:'. Make it conversational, descriptive, and highly engaging. Do not search for news. Set 'is_news' to false." if is_late_night else "Task 2 (Pulse): Adopt the persona of an optimistic, masterful speechwriter. Your mission is to be a reliable source of good feelings and community warmth, uplifting everyone from deep thinkers to those who just want to have fun. Provide a concise, feel-good 2-sentence pulse reflecting the region's true rhythm—this could be a recent local milestone, a fun public event, or a bright observation. NEVER use depressing, melancholic, or overly dramatic phrasing (e.g., avoid words like 'gray veil', 'resilience', or 'struggle'). Keep it warm, engaging, and inviting. DO NOT state the current time. If mentioning an active event or scheduled passage, provide the full time range (e.g., 'until 4 PM') or how much time is left so locals can avoid FOMO. Examples: 'The school's CTE fund just passed, paving an exciting path for the Soo's future!', or 'The Edgar B. Speer is locking through until <i>3:15 PM</i> (about 45 mins left)...'. Wrap all specific locations, subjects, and event times in <i> tags. If an event has a fee, append '($)'. Ground the update in bright, factual UP sensory details. Set 'is_news' to true if sharing a tangible local fact."
