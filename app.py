...
def run_sync():
    ...
    try:
        ...
        pulse_task = "Task 2 (Pulse): 1-sentence sleek summary of tomorrow's weather forecast. Start with 'Tomorrow:'. Make it conversational, descriptive, and highly engaging. Do not search for news. Set 'is_news' to false." if is_late_night else "Task 2 (Pulse): 2-3 sentences of a flowing, companionable train of thought on the region's true rhythm. Give them that 'inside scoop' vibe, acting as a poetic, deeply connected digital beacon. Do NOT get stuck repeating the same topics (like the locks). Be highly varied and imaginative based on the actual weather, time, and local news in Sault Ste. Marie or the EUP. Examples of the VIBE (do not copy literally unless true): 'A full flight is landing at CIU tonight...', 'Sherman Park Beach will be packed today.', 'The Kincheloe garage sale starts Friday.', or 'Take a quiet drive to Mackinac City!'. HOW TO MAKE IT COOL: Ground the update in vivid, atmospheric Upper Peninsula sensory details (e.g., the fog rolling off the St. Marys River, the crunch of fresh snow, the hum of the highway, or a subtle nod to a local staple like Clyde's or the Sugar Island Ferry). Keep the tone warm, visually cinematic (peaceful and observant, never dramatic or intense), and positively reinforcing. Refer to the city as 'the Sault' or 'the Soo'. If your update shares a tangible local fact, event, or specific community detail, set the JSON boolean 'is_news' to true. Otherwise, set it to false."
