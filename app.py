...
def run_sync():
    try:
        ...
        pulse_task = "Task 2 (Pulse): 1-sentence sleek summary of tomorrow's weather forecast. Start with 'Tomorrow:'. Make it conversational, descriptive, and highly engaging. Do not search for news. Set 'is_news' to false." if is_late_night else "Task 2 (Pulse): Adopt the persona of a masterful, poetic speechwriter. You believe deeply in the comforting cyclical patterns of life and the warmth of community. Provide a concise, profound 2-sentence pulse reflecting the region's true rhythm—such as a recent local milestone, a community gathering, or an ongoing event. Weave the current weather into your observation gracefully, framing even rainy or cloudy days with cozy, majestic, or positive language (e.g., 'soft silver morning', 'refreshing breeze') rather than depressing words. DO NOT state the current time. If mentioning an active event or scheduled passage, provide the full time range (e.g., 'until 4 PM') or time remaining so locals can avoid FOMO. Example of the VIBE: 'As the Soo wakes to a soft, silver morning, the Twin Sault Cruisers prepare to bring the hum of engines to the <i>Farmers Market</i> until <i>1:00 PM</i>. Life here moves with the steady current of the river, reminding us that our community spirit always remains bright.' Wrap all specific locations, subjects, and event times in <i> tags. If an event has a fee, append '($)'. Ground the update in vivid, factual UP sensory details. Set 'is_news' to true if sharing a tangible local fact."
