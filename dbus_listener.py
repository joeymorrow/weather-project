...
class BeaconDBusService:
    ...
    def _make_request(self, payload, quiet=False):
        ...
        if not INTERNAL_API_SECRET:
            print("[DBUS ERROR] INTERNAL_API_SECRET is not set. Cannot communicate with app.", flush=True)
            return False
        try:
            ...
            if response.status_code == 200:
                if not quiet:
                    print(f"[DBUS] Successfully executed action: {payload.get('action')}", flush=True)
                return True
            else:
                print(f"[DBUS ERROR] Failed to execute action '{payload.get('action')}'. Status: {response.status_code}, Response: {response.text}", flush=True)
                return False
        except requests.exceptions.RequestException as e:
            print(f"[DBUS ERROR] Communication with app.py failed: {e}", flush=True)
            return False
...
def main():
    print("--- BEACON D-Bus System Listener ---", flush=True)
    if not INTERNAL_API_SECRET:
        print("FATAL: INTERNAL_API_SECRET environment variable not found.", flush=True)
        print("Ensure this script is run with the .env file loaded or as a systemd service with EnvironmentFile set.", flush=True)
        return

    loop = GLib.MainLoop()
    bus = SystemBus()
    bus.publish(BUS_NAME, BeaconDBusService())
    print(f"Service '{BUS_NAME}' published on the System bus.\nListening for D-Bus messages...", flush=True)
    loop.run()
